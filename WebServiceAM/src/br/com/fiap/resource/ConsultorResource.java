...
public class ConsultorResource {
	...
	ConsultorBO bo = new ConsultorBO();
	...
	@Path("/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Consultor get(@PathParam("id") int codigo) {
	...
	public Response post(Consultor consultor, @Context UriInfo uriInfo) {
	...
	@Path("/{id}")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response put(Consultor consultor, @PathParam("id") int codigo) {
		
		consultor.setCodigo(codigo);
		bo.alterar(consultor);
